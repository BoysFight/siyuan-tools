...
(()=>{
    ...
    async function collapseAllBooksThenFocusCurrentBook(element, tab) {
        // 添加防抖，避免频繁触发
        if (collapseAllBooksThenFocusCurrentBook.timer) {
            clearTimeout(collapseAllBooksThenFocusCurrentBook.timer);
        }

        return new Promise((resolve) => {
            collapseAllBooksThenFocusCurrentBook.timer = setTimeout(async () => {
                let currNodeId = "";
                // 等待激活文档加载完毕
                await whenElementExist(()=>{
                    ...
                    if(currNodeId){
                        ...
                        if(currTreeItem) {
                            ...
                            if(currBoxDataUrl){
                                // 获取当前文档所在的路径
                                let currTreeItemPath = currTreeItem.getAttribute("data-path");
                                // 获取所有展开的目录
                    ...
                    // 折叠除当前笔记外的所有笔记
                    const books = document.querySelectorAll("ul.b3-list[data-url]");
