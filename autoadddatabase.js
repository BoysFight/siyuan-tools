...
return (async ()=>{
    ...
    let docBlockId = '20240430105640-waf30sl';

    // 定义数据库块id，必填，根据自己的数据库块id进行修改
    let dbBlockId = '20240918154915-8ktx2i4';
...
return (async ()=>{
    ...
    // 定义数据库块id，必填，根据自己的数据库块id进行修改
    let dbBlockId = '20240915110837-quim9ts';
    ...
    const docsSql = (docBlockId, maxLevel, extendSql) => `
        ...
        where type = 'd'
        -- 查询所有子文档
        and box = '20230322123651-tsuyuox'
...
//自动添加到dailynote数据库3 start

(async () => {
    let isProcessing = false; // 全局处理锁
    const processedNotes = new Set(); // 记录已处理的日记ID
    ...
    window.fetch = async function(...args) {
        ...
        // 如果是创建日记的请求
        if (url === '/api/filetree/createDailyNote' && options?.method === 'POST') {
            const response = await originalFetch.apply(this, args);
            const result = await response.clone().json(); // 获取响应数据

            // 异步处理数据库添加（不阻塞原始请求）
            setTimeout(() => handleNoteAdded(result.data.id), 0);
    ...
    // 2. 实际处理函数
    async function handleNoteAdded(noteId) {
        if (isProcessing || processedNotes.has(noteId)) return;

        isProcessing = true;
        try {
            console.log(`开始处理日记 ${noteId}...`);

            // 配置参数
			//设置日记自动存放的数据库块id
            const dbBlockId = '20231009174308-8c1fkzi';
            ...
            // 检查是否已存在（双重验证）
            const isInResult = await fetchSyncPost("/api/av/getAttributeViewKeys", { id: noteId });
            if (isInResult.data.some(item => item.avID === avID)) {
                console.log('日记已存在数据库中');
                return;
            }
            ...
            await fetchSyncPost('/api/av/addAttributeViewBlocks', {
                avID: avID,
                srcs: [{ id: noteId, isDetached: false }]
            });

            processedNotes.add(noteId); // 记录已处理
            console.log(`成功添加日记 ${noteId} 到数据库`);
        } catch (error) {
